...
// Define the theme definition type using Monaco's IStandaloneThemeData
/* interface ThemeDefinition extends editor.IStandaloneThemeData {
    base: editor.BuiltinTheme; // Explicitly type as BuiltinTheme
    inherit: boolean; 
    rules: Array<{
        token: string;
        foreground?: string;
...
export const defineMonacoThemes = (monaco: typeof import("monaco-editor")) => {
    Object.entries(THEME_DEFINITONS).forEach(([themeName, themeData]) => {
        monaco.editor.defineTheme(themeName, {
            base: themeData.base as editor.BuiltinTheme, // Explicitly cast to BuiltinTheme
            inherit: themeData.inherit,
            rules: themeData.rules.map((rule) => ({
                ...rule,
                foreground: rule.foreground,
